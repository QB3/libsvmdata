...
NAMES = {'finance': 'regression/log1p.E2006.train',
...
def get_X_y(dataset, compressed_path, multilabel, replace=False):
    ...
    y_path = pjoin(LIBSVMDATA_PATH, "%s_target%s" % (NAMES[dataset], ext))
    X_path = pjoin(LIBSVMDATA_PATH, "%s_data.npz" % NAMES[dataset])
    if replace or not os.path.isfile(y_path) or not os.path.isfile(X_path):
        tmp_path = pjoin(LIBSVMDATA_PATH, "%s" % NAMES[dataset])
